...
<div class="mb-8 lg:mb-12">
    <a href="{{ route('admin.cases.index') }}" class="text-primary hover:text-accent font-semibold flex items-center gap-2 transition-colors duration-200 hover:underline inline-block lg:text-lg">
        <span class="text-xl lg:text-2xl">←</span> 
        <span>العودة إلى قائمة القضايا</span>
    </a>
</div>
    ...
    <div class="relative z-10">
        <div class="flex items-start justify-between mb-6 flex-wrap gap-4">
            ...
            <div class="flex items-center gap-3 flex-shrink-0">
                ...
                <a href="{{ route('admin.cases.edit', $case->id) }}" class="btn-attorney-secondary">تعديل</a>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
            <div class="bg-white p-5 rounded-lg border border-gray-200 shadow-sm hover:shadow-md hover:border-primary/30 transition-all duration-200">
                ...
                <p class="text-lg font-bold text-primary">
                    <a href="{{ route('admin.clients.show', $case->client->id) }}" class="hover:text-accent transition-colors">
                        {{ $case->client->name }}
                    </a>
                </p>
            </div>
            ...
            <div class="bg-white p-5 rounded-lg border border-accent/30 shadow-sm hover:shadow-md hover:border-accent/50 transition-all duration-200">
                <div class="flex items-center gap-3 mb-3">
                    ...
                    <p class="text-gray-700 font-semibold">آخر تحديث</p>
                </div>
                <p class="text-lg font-bold text-accent">{{ $case->last_update_text ?? 'لا توجد تحديثات حالياً' }}</p>
            </div>
...
<div class="card-dashboard p-8 lg:p-10 mb-8 lg:mb-12">
    <div class="flex items-center gap-3 lg:gap-4 mb-6 lg:mb-8">
        ...
        <div>
            <h2 class="text-2xl lg:text-3xl font-bold text-primary">تحديث القضية</h2>
            <p class="text-gray-500 text-sm lg:text-base mt-1 lg:mt-2">قم بتحديث نص آخر تحديث للقضية</p>
        </div>
    </div>
    <form method="POST" action="{{ force_https_route('admin.cases.updates.store', $case->id) }}" class="space-y-6">
        @csrf
        <div>
            <label class="block text-gray-700 mb-2 font-semibold">نص التحديث <span class="text-red-500">*</span></label>
            <textarea name="last_update_text" rows="5" class="form-input-attorney" required placeholder="أدخل نص التحديث هنا...">{{ $case->last_update_text ?? '' }}</textarea>
        </div>
        <button type="submit" class="btn-attorney-primary">حفظ التحديث</button>
    </form>
</div>
...
<div class="card-dashboard p-8 lg:p-10">
    <div class="flex items-center gap-3 lg:gap-4 mb-6 lg:mb-8">
        ...
        <div>
            ...
            <p class="text-gray-500 text-sm lg:text-base mt-1 lg:mt-2">استفسارات العميل والردود عليها</p>
        </div>
    </div>
    
    @if($case->inquiries->count() > 0)
        <div class="space-y-4">
                ...
                <div class="bg-gradient-to-br from-accent/10 via-accent/5 to-accent/10 rounded-lg p-5 border-r-4 border-accent shadow-md hover:shadow-lg transition-all">
                    ...
                    <div class="flex items-center gap-3">
                        <div class="icon-circle icon-circle-accent">
                            ...
                            <div>
                                <div class="flex items-center gap-2 mb-1 flex-wrap">
                                    ...
                                    <p class="font-bold text-lg text-primary">{{ $inquiry->client->name }}</p>
                                </div>
                        ...
                        <a href="{{ route('admin.inquiries.show', $inquiry->id) }}" 
                           class="btn-attorney-secondary text-sm px-4 py-2">عرض التفاصيل</a>
                    </div>
                    <div class="bg-white rounded-lg p-4 mb-3 border border-accent/30">
                        <p class="text-gray-800 leading-relaxed text-base">{{ Str::limit($inquiry->message, 150) }}</p>
                    </div>
                        ...
                        <div class="bg-gradient-to-br from-primary/10 via-primary/5 to-accent/10 rounded-lg p-5 border-r-4 border-primary shadow-md mt-3">
                            ...
                            <div class="bg-white rounded-lg p-4 border border-primary/20">
                                <div class="flex items-start gap-2">
                                    ...
                                    <p class="text-gray-800 leading-relaxed text-base pr-2 flex-1">{{ Str::limit($inquiry->reply, 150) }}</p>
                                </div>
                            </div>
                        </div>
                    @else
    ...
    @else
        <div class="text-center py-12 bg-gradient-to-br from-accent/10 to-accent/5 rounded-lg border border-accent/30">
            ...
            <p class="text-gray-600">لا توجد استفسارات من العميل على هذه القضية</p>
        </div>
    @endif
</div>
