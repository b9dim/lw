    ...
    <form method="POST" action="{{ force_https_route('admin.cases.store') }}" class="space-y-6">
        ...
        <div>
            ...
            <select name="status" class="form-input-attorney @error('status') border-red-500 @enderror" required>
                ...
                <option value="قيد المحاكمة" {{ old('status') == 'قيد المحاكمة' ? 'selected' : '' }}>قيد المحاكمة</option>
                <option value="منتهية" {{ old('status') == 'منتهية' ? 'selected' : '' }}>منتهية</option>
                <option value="ملغاة" {{ old('status') == 'ملغاة' ? 'selected' : '' }}>ملغاة</option>
            </select>
            @error('status')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        ...
        <div>
            ...
            <textarea name="description" rows="5" 
                      class="form-input-attorney @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
            @error('description')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="flex gap-4 pt-4">
            <button type="submit" class="btn-attorney-primary">إضافة القضية</button>
